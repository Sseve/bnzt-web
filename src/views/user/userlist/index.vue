    ...
    this.$confirm("是否添加用户" + this.addForm.name, "提示", {
      ...
      AddUser(this.addForm).then(response => {
        if (response.code === 200) {
          this.$message({message: "添加用户: " + this.addForm.name + " 成功!", type: "success"})
        } else {
          this.$message({message: "添加用户: " + this.addForm.name + " 失败!", type: "warning"})
        }
  ...
  delUser(row) {
    this.$confirm("是否删除" + row.name, "提示", {
      ...
      DelUser(row).then(response => {
        if (response.code === 200) {
          this.$message({message: "删除用户: " + row.name + " 成功!", type: "success"})
        } else {
          this.$message({message: "删除用户: " + row.name + " 失败!", type: "warning"})
        }
      })
    })
  },
  updUser() {
    this.$confirm("是否更新用户" + this.updForm.name, "提示", {
      ...
      UpdateUser(this.updForm).then(response => {
        if (response.code === 200) {
          this.$message({message: this.updForm.name + "更新成功" + " 成功!", type: "success"})
        } else {
          this.$message({message: this.updForm.name + "更新失败" + " 失败!", type: "warning"})
        }
